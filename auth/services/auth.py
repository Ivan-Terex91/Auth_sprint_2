...
class TokenService:
    ...
    def _get_refresh_token(self, token: str, validate_exp=True) -> RefreshToken:
        refrest_token = self.session.query(RefreshToken).filter(RefreshToken.token == token).first()
        if not refrest_token:
            raise RefreshTokenNotFound("Refresh token not found")

        if validate_exp and refrest_token.exp.replace(tzinfo=timezone.utc) < datetime.now(
            tz=timezone.utc
        ):
